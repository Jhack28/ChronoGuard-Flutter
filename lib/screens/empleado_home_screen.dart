      ...
      showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            title: Text('Solicitar Permiso', style: TextStyle(color: Colors.teal[900])),
                    ...
                    children: [
                      DropdownButtonFormField<String>(
                        ...
                        items: [
                          ...
                          'eventos familiares'
                        ].map((permiso) => DropdownMenuItem(value: permiso, child: Text(permiso))).toList(),
            ...
            actions: [
              ...
              ElevatedButton(
                ...
                onPressed: () async {
                  if (tipoPermiso == null) {
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Seleccione un tipo de permiso')));
                    return;
                  }
                  final permisoData = {
                  ...
                  try {
                    final idTipoPermiso = await ApiService.crearPermiso(permisoData);
                    await ApiService.crearNotificacionEmpleado({
                      'ID_Usuario': widget.idUsuario,
                      'ID_EstadoPermiso': 1,
                      'Mensaje': 'Solicitud de permiso enviada: $tipoPermiso',
                      'FechaEnvio': DateTime.now().toIso8601String().substring(0, 10),
                      'Estado': 'Pendiente',
                    });
                    ...
                    Navigator.pop(context);
                    ScaffoldMessenger.of(context).showSnackBar(
                      const SnackBar(content: Text('Permiso solicitado correctamente')),
                    );
                  } catch (e) {
                    ScaffoldMessenger.of(context).showSnackBar(
